...
def is_start_page(text):
    ...
    # Ordinary first pages should not contain personnummer. Later pages usually do.
    if contains_personnummer(text):
        return False
...
def process_pdf(args):
    ...
    try:
        # --- scan first pages for readability ---
        for i in range(min(3, len(reader.pages))):
        ...
        for i in range(len(reader.pages)):
            ...
            text = text[:5000]  # memory protection

            # your existing logic hooks
            if is_start_page(text):
                has_cert = True

            # collect personnummer candidates (reuse your existing function)
            pnr_candidates = extract_personnummer_candidates(text)
            if pnr_candidates:
                personnummer_values.extend(pnr_candidates)

        if not has_cert:
            return ("NO_CERT", pdf_path)

        # --- consensus ---
        personnummer = consensus(personnummer_values)

        # --- metadata ---
        filename = os.path.basename(pdf_path)
        folder = os.path.basename(os.path.dirname(pdf_path))

        # ✅ IMPORTANT: return data instead of writing Excel here
        return ("OK", pdf_path, filename, folder, personnummer)

    except Exception as e:
        return ("CRASH", pdf_path, str(e))
...
def process_all(input_root, output_root):
    ...
    ws.append(["Filename", "Folder", "Personnummer"])
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if isinstance(result, tuple):
                                ...
                                if status == "OK":
                                    _, path, filename, folder, pnr = result
                                    ws.append([filename, folder, pnr])
    ...
    validation_path = os.path.join(output_root, VALIDATION_FILENAME)
